...
int TSP::two_opt_sym(int path_length){
...
int TSP::two_opt_nsym(int pathLength ){

	int new_leng = 0, n = get_graph_size(), start = min_path.back();
		...
		for (int i = 1; i < n - 1; i++){
			for (int j = i + 1; j < n ; j++){
				...
				if(get_path_length(min_path, n, start ) < pathLength){

					pathLength = get_path_length(min_path, n, start );
